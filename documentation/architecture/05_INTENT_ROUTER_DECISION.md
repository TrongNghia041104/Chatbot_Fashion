...
flowchart TD
    ...
    C -->|profile+outfit keyword| C1[analyze_then_style\nWorkflow 2 bước]
    C -->|identify keyword| C2[identify_image_item\nVLM mô tả → search catalog]
    C -->|profile/outfit/product keyword| C3[Route ngay]
    C -->|Không có keyword| C4[inspect_image hoặc VLM confidence]
    D --> E[High-precision keyword]
    E -->|Không khớp| F[LLM intent classifier]
    C1 --> G[Semantic fields]
    C2 --> G
    C3 --> G
    C4 --> G
    D --> G
...
**Action hợp lệ trong LLM schema** (tất cả actions `coerce` chấp nhận):
```
search | find_similar | identify_image_item | more
price_check | size_check | stock_check | compare
create_outfit | refine_outfit | style_image_item
read | update | delete_field | clear_all
greeting | thanks | goodbye | redirect | clarify
```
