...
Với `identify_image_item`, backend gửi câu nhận diện trực tiếp, `search_results` và hai follow-up actions: `view_similar_results` chỉ cuộn tới khu card hiện có; `style_image_item` gửi yêu cầu phối đồ và kế thừa image candidates trong session.
